    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <form wire:submit="save">
            <table class="w-full text-sm text-left rtl:text-right text-gray-900 dark:text-gray-100">
                ...
                @endif
                @foreach($consistencies as $key => $consistency)
                    <tr class="bg-white dark:bg-gray-800 {{ !$loop->last ? 'border-b dark:border-gray-700' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            {{ jdate($consistency->created_at)->format('Y/m/d | ساعت H:i:s') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ jdate($consistency->updated_at)->format('Y/m/d | ساعت H:i:s') }}
                        </td>
                        <td class="px-6 py-4 text-right">
                            @if(auth()->user()->isAdmin())
                                <button
                                    class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
                                    wire:click="delete({{ $consistency->id }})">
                                    حذف
                                </button>
                            @endif
                        </td>
