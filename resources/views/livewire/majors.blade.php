            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-900 dark:text-gray-100">
                ...
                @if(auth()->user()->isAdmin())
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @error('form.code') <span class="text-red-600">{{ $message }}</span> @enderror
                        </td>
                        <td class="px-6 py-4">
                            {{ jdate()->format('Y/m/d') }}
                        </td>
                ...
                @foreach($majors as $key => $major)
                    <tr class="bg-white dark:bg-gray-800 {{ !$loop->last ? 'border-b dark:border-gray-700' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            {{ jdate($major->created_at)->format('Y/m/d | ساعت H:i:s') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ jdate($major->updated_at)->format('Y/m/d | ساعت H:i:s') }}
                        </td>
                        <td class="px-6 py-4 text-right">
                            @if(auth()->user()->isAdmin())
                                <button
